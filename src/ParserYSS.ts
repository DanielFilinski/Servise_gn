...
const dateFirstWeek = '2026-06-27' //! дата первого урока (ОБЯЗАТЕЛЬНО суббота — день, по которому приложение ищет урок)
const datesArr = []

// guard: дата первого урока должна быть субботой (day 6).
// Частая ошибка — вписать сюда начало печатного диапазона из буклета («28 июня — …»),
// которое может быть воскресеньем. Приложение ищет урок по дате субботы, поэтому семя обязано быть субботой.
if (new Date(`${dateFirstWeek} 12:00`).getDay() !== 6) {
    throw new Error(`dateFirstWeek=${dateFirstWeek} — не суббота. Урок субботней школы должен начинаться в субботу; впишите дату субботы.`)
}
