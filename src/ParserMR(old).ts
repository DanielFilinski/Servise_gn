...
async function PARSE() {
    ...
    allArr.forEach((p, index) => {
        ...
        if (className.contains(parsePatterns.date)) {
            ...
            const parsedDate = dateParser(`${p.innerText} 2025`, 'd MMMM yyyy', new Date(), { locale: ru });
        ...
        if (className.contains(parsePatterns.meinText) || className.contains(parsePatterns.otherMain)) {
